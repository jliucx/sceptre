...
To test whether calculating p-values using permutation is still valid in high-MOI data, we compare the calibration result of sceptre using conditional resampling and permutation.

**Conditional resampling**

```{r echo=FALSE}
knitr::include_graphics("output/trans_singleton_threshold_author/plot_run_calibration_check.png", error = FALSE)
```

**Permutation**

```{r}
knitr::include_graphics("output/trans_singleton_permute/plot_run_calibration_check.png", error = FALSE)
```

```{r}
results_run_calibration_check <- readRDS("docs/output/trans_singleton_permute/results_run_calibration_check.rds")
filtered_data <- results_run_calibration_check %>% filter(significant == TRUE)
grna_count <- filtered_data %>%
  group_by(grna_id) %>%
  summarise(count = n()) %>%
...
```{r}
pval<-read.csv("/project/xuanyao/jiaming/Getting_started/output/result_calibration_2.csv", header = TRUE, row.names = 1)
```

```{r}
pval_vector <- as.vector(as.matrix(read.csv("/project/xuanyao/jiaming/Getting_started/output/result_calibration_2.csv", header = TRUE, row.names = 1)))
```
...
```{r}
pval_individual <- pval[1, ]
vector_individual <- as.vector(as.matrix(pval_individual))
gg_qqplot(vector_individual)
```

```{r}
pval_flat <- as.vector(as.matrix(pval))  # Flatten the matrix
rownames_flat <- rep(rownames(pval), each = ncol(pval))  # Corresponding row names
...
```{r}
gg_qqplot(sample(pval_sceptre, size = 300, replace = FALSE))
```
