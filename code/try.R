...
n_cells <- ncol(response_matrix[[1]])
n_genes <- nrow(response_matrix[[1]])
...
resample_and_combine <- function(grna_assignment_matrix, n_permute) {
  ...
  combined_mat <- grna_assignment_matrix

  for (i in 1:n_permute) {
    resampled_mat <- matrix(rbinom(n = length(propensity_matrix),
                                   size = 1,
                                   prob = as.vector(propensity_matrix)),
                            nrow = nrow(propensity_matrix),
                            ncol = ncol(propensity_matrix))


    combined_mat <- rbind(combined_mat, resampled_mat)
  }
  return(combined_mat)
}
...
time_taken <- system.time({
  wilcox_result <- wilcox_test(response_matrix[[1]],grna_assignment_matrix_combined, n_permute)
})
...
save_path <- "wilcox_result.rds"

# Save the object
saveRDS(wilcox_result, file = save_path)
