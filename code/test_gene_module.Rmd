...
combine_test_statistics <- function(test_result, gene_module_matrix){
  test_result_matrix <- do.call(rbind, test_result)
    ...
    power_sum_1 <- test_result_matrix%*%gene_module_matrix
    power_sum_2 <- test_result_matrix^2%*%gene_module_matrix
    power_sum_3 <- test_result_matrix^3%*%gene_module_matrix
    power_sum_4 <- test_result_matrix^4%*%gene_module_matrix
    power_sum_5 <- test_result_matrix^5%*%gene_module_matrix
    power_sum_6 <- test_result_matrix^6%*%gene_module_matrix
...
```{r}
analysis_gene_module <- function(grna_assignment_matrix, response_matrix, gene_module_index_matrix, n_permute, use_resample, use_approximation){
  ...
  # test statistics 
  test_result <- t_test(response_matrix,grna_assignment_matrix_combined, (n_permute+1))
...
peak_mem_usage <- peakRAM(
  result <- analysis_gene_module(grna_assignment_matrix, response_matrix,
                                 gene_module_index_matrix, n_permute, use_resample, use_approximation)
)
...
pval_vector_2 <- as.vector(as.matrix(read.csv("/project/xuanyao/jiaming/Getting_started/output/result_calibration_use_approximation_10000.csv", header = TRUE, row.names = 1)))
gg_qqplot(pval_vector_2)
```
...
```{r}
pval_vector_3 <- as.vector(as.matrix(read.csv("/project/xuanyao/jiaming/Getting_started/output/result_calibration_use_approximation_2000.csv", header = TRUE, row.names = 1)))
gg_qqplot(pval_vector_3)
```

```{r}
qqplot(pval_vector_2, pval_vector_3, main = "Q-Q Plot of Two Vectors",
...
```{r}
cor(pval_vector_3,pval_vector_2)
```
