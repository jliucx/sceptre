...
library(qqman)
library(ggplot2)
results_run_calibration_check <- readRDS("/project/xuanyao/jiaming/Getting_started/output/sceptre_outputs_cis/results_run_calibration_check.rds")
```
...
```{r}
library(dplyr)
results_run_calibration_check <- readRDS("/project/xuanyao/jiaming/Getting_started/output/sceptre_outputs_trans_singleton/results_run_calibration_check.rds")
...
We noticed that the calibration check result when we perform "trans analysis"(it also contains cis analysis) is inflated. To see whether there might be problem with NT gRNA design, we find that NTC-3 and NTC-12 contribute to over 50% percent of false positive discoveries.
...
```{r echo=FALSE}
knitr::include_graphics("images/trans_singleton.png", error = FALSE)
```

[text summary](images/trans_singleton.txt)

### Trans analysis with gRNA method using mixture model (singleton)(omitting NTC-3&12)

```{r echo=FALSE}
knitr::include_graphics("images/trans_calibration_check_mixture_omitNTC3.png", error = FALSE)
```

[text summary](images/trans_mixture_omit.txt)

### Trans analysis with gRNA method using mixture model
...
### Trans analysis with gRNA method using mixture model (omitting NTC-3&12)

```{r echo=FALSE}
knitr::include_graphics("images/trans_mixture_omit.png", error = FALSE)
```

[text summary](images/trans_mixture_omit2.txt)
...
data_matrix <- matrix(c("67/155595", "292/976820", "51/1027281", "15/126552", "158/652822", "0/684410"), nrow = 2, byrow = TRUE)

# Set row and column names
rownames(data_matrix) <- c("original", "removing NTC3&12")
colnames(data_matrix) <- c("mixture(singleton)", "mixture(union)", "threshold")
...
kable(contingency_table, format = "html", table.attr = "class='table table-striped'",
      col.names = c("Mixture(Singleton)", "Mixture(Unicon)", "Threshold"), row.names = TRUE,
...
## Conditional resampling algorithm

```{r echo=FALSE}
knitr::include_graphics("images/sceptre_illustration.png", error = FALSE)
```

```{r echo=FALSE}
knitr::include_graphics("images/methodology.png", error = FALSE)
```
