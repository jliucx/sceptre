...
t_test <- function(response_matrix, grna_assignment_matrix) {
  ...
  # Start timing for the first block
  start_time <- Sys.time()
  ...
  end_time <- Sys.time()
  print(paste("Time for setup:", end_time - start_time))
  
  # Start timing for the next block
  start_time <- Sys.time()
  
  grna_infected_cell_count <- grna_assignment_matrix %*% matrix(rep(1, n_cells), nrow = n_cells)
  end_time <- Sys.time()
  print(paste("Time for infected cell count:", end_time - start_time))
  
  start_time <- Sys.time()
  mean_expression_trt <- grna_assignment_matrix %*% t(response_matrix) / as.vector(grna_infected_cell_count)
  
  end_time <- Sys.time()
  print(paste("Time for infected cell mean:", end_time - start_time))
  
  # Repeat this for other blocks of code
  start_time <- Sys.time()

  mean_expression_squared_trt <- grna_assignment_matrix %*% t(response_matrix^2) / as.vector(grna_infected_cell_count)
  end_time <- Sys.time()
  print(paste("Time for infected cell mean squared:", end_time - start_time))
  ...
  end_time <- Sys.time()
  print(paste("Time for variance calculation (treated):", end_time - start_time))
  
  # Continue for other calculations
  start_time <- Sys.time()
  ...
  print(paste("Time for control mean calculation:", end_time - start_time))
  ...
  end_time <- Sys.time()
   ...
   print(paste("Time for control mean squared:", end_time - start_time))
   
   start_time <- Sys.time()
   var_expression_control <- mean_expression_squared_control - mean_expression_control^2
   end_time <- Sys.time()
   
   print(paste("Time for control variance:", end_time - start_time))
   
   start_time <- Sys.time()

  t_statistics <- (mean_expression_trt - mean_expression_control) /
    sqrt(var_expression_trt / as.vector(grna_infected_cell_count) + var_expression_control / as.vector(grna_control_cell_count))
  
  end_time <- Sys.time()
  print(paste("Time for t-statistics calculation:", end_time - start_time))
...
grna_assignment_matrix_combined <- permutation_or_resampling(grna_assignment_matrix, n_permute, use_resample)
  # test statistics from permutation
  t_result_permuted <- t_test(response_matrix,grna_assignment_matrix_combined)
```
...
```{r}
pval_vector_10000 <- as.vector(as.matrix(read.csv("/project/xuanyao/jiaming/Getting_started/output/calibration_wilcoxon_10000_no_approximation.csv", header = TRUE, row.names = 1)))
gg_qqplot(pval_vector_10000)
```

```{r}
sum(is.na(pval_vector_2000))
```
